        ...
        <div class="card-header">
            <h3 class="card-title">
                <button type="button" class="btn btn-success" data-toggle="modal" data-target="#createInvoice">
                    <span data-toggle="tooltip" data-placement="left" title="افزودن فاکتور برگشت از خرید">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-12">
                        <select id="index_invoice_row"
                            onChange="fetchDataAsPaginate('index_invoice_search','/return-buy-factor',1,this.value,'index_count','myData','index_pagination')"
    ...
    <script>
            ...
            $('#index_invoice_search').on('keyup', function() {
                ...
                serach_data
                    (
                        value,
                        row,
                        "/index-search-return-buy-factors",
                        "myData",
                        "index_pagination"
                    );
            });
        ...
        $(document).ready(function() {
            $("#index_factors_table").on('click', '.indexRow', function() {
                ...
                var return_buy_factor_id = currentRow.find("td:eq(0)").html();

                $.ajax({
                    type: "GET",
                    url: "/return-buy-factor-detail/" + return_buy_factor_id + "/edit",
                    dataType: "json",
                    success: function(response) {
                        if (response.status == 404) {
                            ...
                            $('#selectData').html(response.return_buy_factor_by_id);
