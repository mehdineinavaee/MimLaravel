    ...
    <div class="modal-dialog modal-dialog-centered" style="min-width: 60%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editInfoLabel">ویرایش پیش فاکتور فروش</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    <input type="hidden" id="edit_sell_factor_advanced_id">
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        <div class="form-group mb-3">
                            <label for="edit_applicant">درخواست کننده</label>
                            <select id="edit_applicant" name="edit_applicant" class="form-control select2"
                                style="width: 100%;">
                                <option value="" selected>درخواست کننده را انتخاب کنید</option>
                            ...
                            <div id="edit_applicant_error" class="invalid-feedback"></div>
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        <div class="form-group mb-3">
            ...
            <div class="modal-footer" style="justify-content: space-between;">
                ...
                <div>
                    ...
                    <button type="button" class="btn btn-primary updateSellFactorAdvanced">تأیید</button>
                </div>
            </div>
        </div>
    </div>
...
<!-- End Modal -->
@include('buy-sell.sell-factor-advanced-details.edit')
    ...
    <script>
        ...
        function editCustomerType(divId, element) {
            if (element == 2) {
                ...
                $(edit_applicant).prop('selectedIndex', 0).change();
                document.getElementById("edit_applicant").disabled = true;
                $("#edit_applicant_error").text("");
                $("#edit_applicant").removeClass("is-invalid");
            } else {
                document.getElementById(divId).style.display = 'none';
                document.getElementById("edit_applicant").disabled = false;
            }
        }

        $(document).on('click', '.edit_sell_factor_advanced', function(e) {
            e.preventDefault();
            var sell_factor_advanced_id = $(this).val();
            // console.log(sell_factor_advanced_id);

            $.ajax({
                type: "GET",
                url: "/sell-factor-advanced/" + sell_factor_advanced_id + "/edit",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        $("#edit_sell_factor_advanced_id").val(sell_factor_advanced_id);
                        $("#edit_national_code").val(response.sell_factor_advanced.national_code);
                        $("#edit_viator").val(response.sell_factor_advanced.viator);
                        $("#edit_tel").val(response.sell_factor_advanced.tel);
                        $("#edit_address").val(response.sell_factor_advanced.address);
                        $('#edit_customer_type').val(response.sell_factor_advanced.customer_type)
                            .change();
                        $('#edit_applicant').val(response.sell_factor_advanced.applicant_id).change();
                        $("#edit_factor_no").val(response.sell_factor_advanced.factor_no);
                        $("#edit_factor_date").val(response.sell_factor_advanced.factor_date);
                        $('#edit_broker').val(response.sell_factor_advanced.broker_id).change();
                        $("#edit_commission").val(response.sell_factor_advanced
                            .commission);
                        $("#edit_settlement_deadline").val(response.sell_factor_advanced
                            .settlement_deadline);
                        $("#edit_settlement_date").val(response.sell_factor_advanced
                            .settlement_date);
                    }
                },
            });
        ...
        $(document).on("click", ".updateSellFactorAdvanced", function(e) {
            e.preventDefault();
            var sell_factor_advanced_id = $("#edit_sell_factor_advanced_id").val();
            ...
            $.ajax({
                type: "PUT",
                url: "/sell-factor-advanced/" + sell_factor_advanced_id,
                data: data,
                dataType: "json",
                success: function(response) {
                    // console.log(response);
