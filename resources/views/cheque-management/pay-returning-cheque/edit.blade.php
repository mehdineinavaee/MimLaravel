    ...
    <div class="modal-dialog modal-dialog-centered" style="min-width: 700px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <input type="hidden" id="edit_pay_returning_cheque_id">
                    <div class="col-lg-6 col-md-6 col-sm-12">
                        <div class="form-group mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <div id="edit_form_date_error" class="invalid-feedback" style="margin-right:38px;">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                <div>
                    ...
                    <button type="button" class="btn btn-primary updatePayReturningCheque">تأیید</button>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    <script>
        $(document).on('click', '.edit_pay_returning_cheque', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                type: "GET",
                url: "/pay-returning-cheque/" + pay_returning_cheque_id + "/edit",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        $("#edit_serial_number").val(response.pay_returning_cheque.serial_number);
                        $('#edit_total').val(new Intl.NumberFormat().format(response
                            .pay_returning_cheque
                            .total));
                        $('#edit_wage').val(new Intl.NumberFormat().format(response.pay_returning_cheque
                            .wage));
                        $("#edit_due_date").val(response.pay_returning_cheque.due_date);
                        $('#edit_bank_account_details').val(response.pay_returning_cheque
                                .bank_account_id)
                            .change();
                        $("#edit_receiver").val(response.pay_returning_cheque.receiver);
                        $("#edit_considerations").val(response.pay_returning_cheque.considerations);
                    }
                },
            });
        });

        $(document).on("click", ".updatePayReturningCheque", function(e) {
            e.preventDefault();
            var pay_returning_cheque_id = $("#edit_pay_returning_cheque_id").val();
            ...
            $.ajax({
                type: "PUT",
                url: "/pay-returning-cheque/" + pay_returning_cheque_id,
                data: data,
                dataType: "json",
                success: function(response) {
                    // console.log(response);
