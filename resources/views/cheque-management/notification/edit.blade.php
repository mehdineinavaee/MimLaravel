    ...
    <div class="modal-dialog modal-dialog-centered" style="min-width: 700px">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editInfoLabel">ویرایش اعلام وصول چک های خوابانده شده</h5>
            ...
            <div class="modal-footer">
                <div>
                    ...
                    <button type="button" class="btn btn-primary updateNotification">تأیید</button>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    <script>
        $(document).on('click', '.edit_notification', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: "/notification/" + notification_id + "/edit",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        $("#edit_mark_back").val(response.notification.mark_back);
                        $("#edit_serial_number").val(response.notification.serial_number);
                        $('#edit_total').val(new Intl.NumberFormat().format(response.notification
                            .total));
                        $("#edit_due_date").val(response.notification.due_date);
                        $('#edit_bank_account_details').val(response.notification.bank_account_id)
                            .change();
                        $("#edit_payer").val(response.notification.payer);
                        $("#edit_considerations").val(response.notification.considerations);
                    }
                },
            });
        });
