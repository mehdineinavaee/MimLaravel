    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editInfoLabel">ویرایش برداشت شرکا</h5>
            ...
            <div class="modal-body">
                <input type="hidden" id="edit_withdrawal_partner_id">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary updateWithdrawal_partner">تأیید</button>
            </div>
        </div>
    </div>
</div>
...
@push('js')
    <script>
        $(document).on('click', '.edit_withdrawal_partner', function(e) {
            e.preventDefault();
            var withdrawal_partner_id = $(this).val();
            // console.log(withdrawal_partner_id);

            $.ajax({
                type: "GET",
                url: "/withdrawal-partner/" + withdrawal_partner_id + "/edit",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        $("#editInfo").modal("show");
                        $("#edit_withdrawal_partner_id").val(withdrawal_partner_id);

                        $('#edit_from_taraf_hesab').val(response.withdrawal_partner
                            .from_taraf_hesab_id).change();
                        $('#edit_to_taraf_hesab').val(response.withdrawal_partner
                            .to_taraf_hesab_id).change();
                        $("#edit_form_date").val(response.withdrawal_partner.form_date);
                        $("#edit_form_number").val(response.withdrawal_partner.form_number);
                        $('#edit_cash_amount').val(new Intl.NumberFormat().format(response
                            .withdrawal_partner
                            .cash_amount));
                        $("#edit_document").val(response.withdrawal_partner
                            .document);
                    }
                },
            });
        ...
        $(document).on("click", ".updateWithdrawal_partner", function(e) {
            e.preventDefault();
            var withdrawal_partner_id = $("#edit_withdrawal_partner_id").val();
            ...
            $.ajax({
                type: "PUT",
                url: "/withdrawal-partner/" + withdrawal_partner_id,
                data: data,
                dataType: "json",
                success: function(response) {
                    // console.log(response);
