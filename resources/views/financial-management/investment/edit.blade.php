    ...
    <div class="modal-dialog modal-dialog-centered"style="min-width: 60%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editInfoLabel">ویرایش پرداخت شرکاء (سرمایه گذاری)</h5>
            ...
            <div class="row m-4">
                <div class="col-lg-6 col-md-6 col-sm-12">
                    <div class="form-group mb-3">
                        <label for="edit_shareholder">نام سهام دار</label>
                        <select id="edit_shareholder" name="edit_shareholder" class="form-control select2"
                            style="width: 100%;">
                        ...
                        <div id="edit_shareholder_error" class="invalid-feedback"></div>
                    </div>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12">
                    <div class="form-group mb-3">
            ...
            <div class="modal-body" style="min-height: 315px">
                <input type="hidden" id="edit_investment_id">
                <!-- Tabs content -->
                <fieldset class="show" id="edit_tab011">
                    <div class="row pt-4">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="form-group mb-3">
                                <label for="edit_cash_register">نام صندوق</label>
                                <input type="text" id="edit_cash_register" name="edit_cash_register"
                                    class="form-control" autocomplete="off" />
                                <div id="edit_cash_register_error" class="invalid-feedback"></div>
                            </div>
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="form-group mb-3">
            ...
            <div class="modal-footer">
                <div>
                    ...
                    <button type="button" class="btn btn-primary updateInvestment">تأیید</button>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    <script>
        $(document).on('click', '.edit_investment', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: "/investment/" + investment_id + "/edit",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        $("#editInfo").modal("show");
                        $("#edit_investment_id").val(investment_id);

                        $('#edit_shareholder').val(response.investment.investor_id).change();
                        $("#edit_form_date").val(response.investment.form_date);
                        $('#edit_cash_amount').val(new Intl.NumberFormat().format(response.investment
                            .cash_amount));
                        $("#edit_cash_register").val(response.investment.cash_register);
                        $("#edit_considerations1").val(response.investment
                            .considerations1);
                        $("#edit_date").val(response.investment.date);
                        $('#edit_bank_account_details').val(response.investment.bank_account_id)
                            .change();
                        ...
                        $('#edit_wage').val(new Intl.NumberFormat().format(response
                            .investment
                            .wage));
                        $("#edit_issue_tracking").val(response.investment.issue_tracking);
                        $("#edit_considerations2").val(response.investment
                            .considerations2);
                    }
                },
            });
        ...
        $(document).on("click", ".updateInvestment", function(e) {
            e.preventDefault();
            var investment_id = $("#edit_investment_id").val();

            var data = {
                shareholder: $("#edit_shareholder").val(),
                form_date: $("#edit_form_date").val(),
                cash_amount: $("#edit_cash_amount").val(),
                cash_register: $("#edit_cash_register").val(),
        ...
        function edit_clearErrors() {
            $("#edit_shareholder_error").text("");
            $("#edit_shareholder").removeClass("is-invalid");
            ...
            $("#edit_cash_register_error").text("");
            $("#edit_cash_register").removeClass("is-invalid");
        ...
        function edit_defaultSelectedValue() {
            $(edit_shareholder).prop('selectedIndex', 0).change();
